...
3. Layer styling
    ...
    - **Note**: we try to make the styling as similar as possible, but know that the [MapLibre](https://maplibre.org/) library we use is not as advanced as for example ArcGIS Pro. So customized colormaps might look slightly different in the NINA Maps Portal. 
...
1. Install the backend architecture
    ...
    - Follow the instructions in the README of the [nina-catalogue](https://github.com/NINAnor/nina-catalogue)
        ...
        - create an admin user
            ...
            Set the admin email and password, and remember them to later login to the Django admin: [localhost:8000/admin/](http://localhost:8000/admin/) 
...
2. Install the frontend map application

    - Follow the instructions in the README of the [nina-map-explorer](https://github.com/NINAnor/nina-map-explorer)
...
| Map application | [localhost:3000](http://localhost:3000) |
...
3. Naturregnskaps API - Upload map layers and metadata 
    ...
    e. Upload the data and metadata using the API
...
#!/bin/bash
exec docker run --rm --pid=host     -v /tmp/.X11-unix:/tmp/.X11-unix:ro -e DISPLAY=$DISPLAY     -v "$PWD":/host --workdir /host     -v /tmp:/tmp     --entrypoint="" "osgeo/gdal:alpine-small-3.6.2" "gdal_translate" "$@"
